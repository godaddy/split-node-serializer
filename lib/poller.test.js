...
describe('lib.poller.Poller', () => {
  ...
  beforeEach(() => {
    poller = new Poller({
      ...
      splitioApiBinding: {
        getSplits: () => {
          return new Promise((resolve, reject) => setTimeout(() => resolve(mockGetSplitsResult), 50))
        },
        getSegmentsForSplits: () => {
          return new Promise((resolve, reject) => setTimeout(() => resolve(mockGetSegmentsResult), 50))
        }
      }
    })
  })
