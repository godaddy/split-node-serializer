...
class SplitioApiBinding {
  ...
  async getSegments ({ splits }) {
    ...
    const promises = allSegmentNames.map(async segmentName => {
      ...
      const results = await this.getAllChanges({ path })
      results.allChanges.forEach(change => {
        console.log(change)
        if (segments[change.name]) {
          segments[change.name].added.push(...change.added)
          segments[change.name].removed.push(...change.removed)
        } else {
          segments[change.name] = change
        }
      })
    })

    // iterate over merged segments and remove any removed keys from added keys?
    // delete removed since we don't need to send that data
...
const splitioApi = new SplitioApiBinding({ splitioApiKey: 'hdftsrhil94tdl6kj6l13ci6f2d3t7cf2pb9' })
async function output() {
  const { splits } = await splitioApi.getSplits()
  const {segments } = await splitioApi.getSegments({ splits })
